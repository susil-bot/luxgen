...
export const SubdomainTester: React.FC = () => {
  ...
  const handleTenantSwitch = (tenant: TenantConfig) => {
    ...
    // Simulate subdomain switching
    const currentHost = window.location.hostname;
    const port = window.location.port ? `:${window.location.port}` : '';
    
    if (currentHost === 'localhost' || currentHost === '127.0.0.1') {
      // For localhost, we'll use URL parameters to simulate subdomain
      const newUrl = `${window.location.protocol}//${currentHost}${port}${window.location.pathname}?tenant=${tenant.slug}`;
      window.history.pushState({}, '', newUrl);
    } else {
      // For production, redirect to actual subdomain
      const newUrl = `${window.location.protocol}//${tenant.subdomain}.${currentHost.split('.').slice(1).join('.')}${port}${window.location.pathname}`;
